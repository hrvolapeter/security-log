...
module Analyses.InjectionSpec (main, spec) where
...
spec = do
    describe "sql" $ do
        describe "keywords " $ do
            it "detects SQL select" $
                analyse (build "SELECT ") `shouldSatisfy` isJust

            it "detects or" $
                analyse (build "or ") `shouldSatisfy` isJust

            it "detects upper case or" $
                analyse (build "OR ") `shouldSatisfy` isJust
            ...
            it "detects and" $
                analyse (build "%26%26 ") `shouldSatisfy` isJust

            it "detects UNION" $
                analyse (build "UNION ") `shouldSatisfy` isJust

            it "detects ORDER BY" $
                analyse (build "ORDER BY ") `shouldSatisfy` isJust

        describe "database detection" $ do
            it "detects comments #1" $
                analyse (build "/*") `shouldSatisfy` isJust

            it "detects comments #2" $
                analyse (build "--") `shouldSatisfy` isJust

            it "detects sql limit" $
                analyse (build "LIMIT ") `shouldSatisfy` isJust
        ...
        describe "blind injection" $ do
            ...
            it "detects time delay" $
                analyse (build "sleep(") `shouldSatisfy` isJust
        ...
        describe "detection prevention" $ do
            ...
            it "detects urlencoded" $
                analyse (build "%27%20UNION%20SELECT%20password%20FROM%20Users%20WHERE%20name%3D%") `shouldSatisfy` isJust
    ...
    describe "command injection" $ do
        it "detects /bin" $
            analyse (build "/bin") `shouldSatisfy` isJust
