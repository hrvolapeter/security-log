...
analyse a
  ...
  where
    ...
    checkIfAllElementIsAllowed (TagOpen name attributes) = name `elem` allowedElemets &&
      Prelude.foldl (\x xs -> x && checkIfAtttributeIsAllowed xs) True attributes
    ...
    checkIfValuesIsAllowed value = Prelude.foldl (\x xs -> x && not (xs `isInfixOf` value)) True disallowedValues
